...
(function() {
  ...
  Commit = require('gulp-gh-pages/node_modules/gift/lib/commit');
  ...
  exports.Head = Head = (function(superClass) {
    ...
    Head.current = function(repo, callback) {
      return fs.readFile(repo.dot_git + "/HEAD", "utf8", function(err, data) {
        var branch, m, ref;
        if (err) {
          return callback(err);
        }
        ...
        return fs.readFile(repo.dot_git + "/refs/heads/" + branch, "utf8", function(err, id) {
          return Commit.find(repo, id.trim(), function(err, commit) {
            if (err) {
              return callback(err);
            }
